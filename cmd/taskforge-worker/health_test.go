...
func TestHealth_LivenessIsUnconditional(t *testing.T) {
	server := newHealthServer(":0", discard(), nil,
		failingCheck("session"), failingCheck("control_plane"),
		failingCheck("broker"), failingCheck("objectstore"))
...
func TestHealth_ReadinessIsOKWhenEveryCheckPasses(t *testing.T) {
	server := newHealthServer(":0", discard(), nil,
		okCheck("session"), okCheck("control_plane"),
		okCheck("broker"), okCheck("objectstore"))
	...
	require.Equal(t, map[string]string{
		"session": "ok", "control_plane": "ok", "broker": "ok", "objectstore": "ok",
	}, components)
}
...
//
// objectstore is the one M6C added. M5C made this process depend on the object
// store -- a large result is uploaded before the attempt reports success -- but
// readiness never checked it, so a worker that could not reach it reported
// itself ready and then failed every large-result job. The per-dependency
// isolation below is what proves the new check neither masks nor is masked by
// the three that were already there.
func TestHealth_ReadinessFailsPerDependencyAndNamesIt(t *testing.T) {
	names := []string{"session", "control_plane", "broker", "objectstore"}
...
func TestHealth_MetricsIsValidExpositionFormat(t *testing.T) {
	...
	server := newHealthServer(":0", discard(), m, okCheck("session"))
		...
		for _, metric := range family.GetMetric() {
			...
			for _, label := range metric.GetLabel() {
				if label.GetName() == "service" {
					require.Equal(t, "taskforge-worker", label.GetValue())
					found = true
				}
			}
...
func TestHealth_MetricsIsAbsentWithoutInstrumentation(t *testing.T) {
	server := newHealthServer(":0", discard(), nil, okCheck("session"))
...
// The object-store check is the specific residual M6C closed, so it gets its
// own assertion rather than only being one row in the table above: a worker
// that cannot reach the bucket must report NOT ready, with the other three
// components still reporting ok.
func TestHealth_ObjectStoreFailureIsReportedInIsolation(t *testing.T) {
	server := newHealthServer(":0", discard(), nil,
		okCheck("session"), okCheck("control_plane"),
		okCheck("broker"), failingCheck("objectstore"))

	rec := serve(t, server, "/readyz")
	require.Equal(t, http.StatusServiceUnavailable, rec.Code,
		"a worker that cannot reach the object store cannot report large results "+
			"and is not ready to take work")
